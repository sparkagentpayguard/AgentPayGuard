...
> **📌 声明**：本指南仅为参考。Role A 可以选择使用任何支持多签和权限控制的合约方案（不限于本指南提及的工具）。关键是完成多签钱包部署和冻结操作，具体实现方式完全由你们决定。
>
> **文档分工**：中文内部交接与交付物清单（交付给角色B/C、部署指南、操作清单等）见 [docs/multisig/](../multisig/README.md)。
