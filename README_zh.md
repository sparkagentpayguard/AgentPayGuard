...
graph TB
    User[👤 用户请求<br/>自然语言]
    API[🌐 API 服务<br/>/api/ai-pay, /api/pay]
    Identity[🔐 Agent 身份<br/>KitePass / AA SDK]
    AI[🤖 AI 意图解析器<br/>解析 + 风险评估]
    Policy[🛡️ 策略引擎<br/>规则 + AI风险 + ML + 冻结]
    Pay[💸 支付执行<br/>EOA 或 AA 路径]
    Chain[⛓️ Kite 链<br/>稳定币转账]
    
    Multisig[👥 SimpleMultiSig<br/>2/3 多签]
    Freeze[🚫 冻结合约<br/>紧急兜底]
...
| **🛡️ 策略引擎** | [`src/lib/policy.ts`](src/lib/policy.ts) | 白名单/限额/AI风险评估/链上冻结检查 | ✅ |
...
| 模块 | 文件 | 功能 | 状态 |
|------|------|------|------|
| **⚙️ 配置管理** | [`src/lib/config.ts`](src/lib/config.ts) | 环境变量加载与验证 | ✅ |
...
| 策略类型 | 描述 | 默认值 | 状态 |
|---------|------|--------|------|
| **📝 收款地址白名单** | 只允许支付给预先登记的供应商/合约地址 | 可配置 | ✅ |
| **💰 单笔上限** | 每笔最多支付 `MAX_AMOUNT` | 可配置 | ✅ |
| **📅 周期限额** | 每日总额不超过 `DAILY_LIMIT` | 可配置 | ✅ |
| **⏰ 授权有效期** | 超过有效期的支付请求自动失效 | 可选 | ✅ |
| **🤖 AI风险评估** | 基于支付目的、金额、历史模式的风险评分（0-100） | 0-100 分数 | ✅ |
| **🚨 AI风险阈值** | 可配置最大风险分数和自动拒绝的风险等级 | 默认：70, ["high"] | ✅ |
| **🚫 链上冻结检查** | 实时检查多签冻结状态（强依赖模式） | 始终检查 | ✅ |
| **🧠 ML风险评估** | XGBoost 模型 + 孤立森林异常检测，59维特征工程 | 可选（MVP） | ⚠️ |
...
| # | 特性 | 描述 | 影响 |
|---|------|------|------|
| **1** | **🤖 真正的AI Agent** | 不仅仅是自动化脚本，而是能理解自然语言、进行风险评估的智能系统 | 高 |
| **2** | **🔒 多层安全防护** | 传统规则 + AI风险评估 + ML风险检测（可选）+ 链上冻结检查 | 关键 |
| **3** | **🚀 端到端工作流** | 从自然语言请求到链上执行的完整闭环 | 高 |
| **4** | **📊 可验证的AI决策** | AI风险评估透明可解释，提供风险理由和建议 | 中 |
| **5** | **🔄 优雅降级** | 无AI API时自动使用回退解析器，保证系统可用性 | 高 |
| **6** | **🌐 多AI提供商支持** | 支持 DeepSeek、Gemini、OpenAI、Claude、Ollama 等多种提供商 | 中 |
| **7** | **🔐 安全第一** | 严格的环境变量管理、敏感信息保护、多签冻结机制 | 关键 |
...
graph TB
    subgraph Frontend["🌐 前端层"]
        User[👤 用户 / 前端<br/>自然语言请求]
    end
    
    subgraph Backend["⚙️ 后端层"]
        API[🌐 API 服务<br/>src/server.ts<br/>/api/ai-pay, /api/pay, /api/ai-chat]
        Identity[🔐 Agent 身份<br/>src/lib/kite-agent-identity.ts<br/>KitePass / AA SDK]
        AI[🤖 AI 意图解析器<br/>src/lib/ai-intent.ts<br/>parseAndAssessRisk]
        Policy[🛡️ 策略引擎<br/>src/lib/policy.ts<br/>evaluatePolicyWithAI]
        Pay[💸 支付执行<br/>src/lib/run-pay.ts<br/>runPay]
    end
    
    subgraph PolicyDetails["策略详情"]
        Rules[📋 传统规则<br/>白名单、限额、冻结]
        AIRisk[🧠 AI 风险评估<br/>分数 0-100]
        MLRisk[🔬 ML 风险评估<br/>可选：XGBoost + 异常检测]
    end
    
    subgraph PaymentPaths["支付路径"]
        EOA[🔗 EOA 路径<br/>src/lib/erc20.ts<br/>transferErc20]
        AA[⚡ AA 路径<br/>src/lib/kite-aa.ts<br/>sendErc20ViaAA]
    end
    
    subgraph Blockchain["⛓️ 区块链层"]
        Chain[Kite 链<br/>稳定币转账<br/>链上审计轨迹]
    end
    
    subgraph Emergency["🚨 紧急兜底"]
        Multisig[👥 SimpleMultiSig<br/>2/3 多签]
        Freeze[🚫 冻结合约<br/>阻止支付]
    end
    ...
    style Pay fill:#fff9c4,stroke:#f9a825,stroke-width:2px
    style Chain fill:#ffebee,stroke:#c62828,stroke-width:3px
    style Multisig fill:#ffebee,stroke:#c62828,stroke-width:2px
    style Freeze fill:#ffebee,stroke:#c62828,stroke-width:2px
```
